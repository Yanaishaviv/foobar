...
def solution(number_list):
	...
	for number in number_list:
		if abs(number*largest_product) < abs(number):
			largest_product = number
		...
		if number < 0:
			...
			biggest_negative = max(biggest_negative, number)
	if largest_product < 0:
		if largest_product != biggest_negative:
			largest_product /= biggest_negative
		else:
			largest_product = max(max(number_list), largest_product)
	return str(int(largest_product))
